...
NOTE on identifiability
Let F: P → O be the forward map from parameter space P ⊂ ℝ⁶ (e.g., Re, Le, Bl, Rms, Mms, Cms) to the observed electrical impedance Ze(ω) ∈ O. The map F is injective – in theory, a unique parameter vector produces a given impedance curve.

However, the Hessian of the misfit functional (or the Fisher information matrix) has a very large condition number. The likelihood landscape is extremely flat along certain directions in P, corresponding to the nearly‑indistinguishable trade‑off between, e.g., Rms and Cms. Consequently, the problem is ill‑conditioned in practice: small measurement noise or numerical rounding leads to large parameter variations.

Geometrically: The preimage F⁻¹({Ze_obs}) is a single point (identifiability), but the level sets of the negative log‑likelihood are long, narrow valleys. The solver finds the MAP estimate, and the posterior covariance reflects the flatness via wide confidence intervals.

To condition the problem (lift the flat directions): Add independent observables – e.g., impedance with added mass, near‑field pressure, or laser displacement. Each new observable F_j: P → O_j defines an additional forward map. The joint forward map (F, F₁, …, F_k) has a Hessian with improved condition number. The adjoint of the joint map projects sensitivity back to parameter space, collapsing the flat directions. In Bayesian terms, the posterior covariance shrinks, and the confidence intervals tighten.

For practical use: A single impedance measurement yields a ballpark estimate suitable for most engineering tasks (enclosure design, crossover simulation). The intrinsic flatness is honestly reported via the confidence intervals. To eliminate the remaining ambiguity (e.g., for research or high‑precision characterisation), the framework scales seamlessly to multiple observables – just append their residuals, and the same Bayesian machinery computes the joint posterior.
