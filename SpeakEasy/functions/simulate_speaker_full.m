...
% Returns a structured result and optionally plots.
    ...
    H_rad = (1i .* w .* rho .* Sd) ./ (2*pi*opts.r_ref);
    ...
    % -------------------- summary prints --------------------
    fprintf('\n=== COMPARISON SUMMARY ===\n');
    fprintf('radiationModel: %s | Sd = %.6g m^2\n', opts.radiationModel, Sd);
    ...
    fprintf('Ma_at_res = %.6g kg | M_eff = %.6g kg | Re[Zm]@res = %.6g | Q_ms = %.4f\n', ...
...
function scenario = solve_scenario(common, cfg)
    ...
    if cfg.useBox
        ...
        C_box   = Vb / (common.rho * common.c^2 * common.Sd^2);
        Cms_box = (Cms_free * C_box) / (Cms_free + C_box);
        R_box   = alpha * (common.rho * common.c * common.Sd^2 / Vb);
