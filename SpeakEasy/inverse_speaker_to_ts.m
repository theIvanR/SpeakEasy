% Inverse Problem Implementation (run after bode plot generated)
clear; clc; close all;
addpath('functions')

%% 1: Transducer Configuration
cfg = struct();
cfg.csvFile      = "impedance_bode.csv";
cfg.cohThreshold = 0.8;
cfg.stride       = 1; % Reduction ratio
cfg.fMin         = 30; % Hz
cfg.fMax         = 300; % Hz
...
p = struct( ...
    ...
    'Bl',   10,       ... % T*m, force factor B*l (fitted)
    'Rms',  2.0,      ... % N*s/m, mechanical resistance of suspension (fitted)
    'Mms',  10.4e-3,  ... % kg, moving mass of diaphragm + coil (fitted)
    'Cms',  77e-6,    ... % m/N, mechanical compliance of suspension (derived from Mms & fs)
    'Sd',   132e-4,   ... % m^2, effective diaphragm area (fixed)
